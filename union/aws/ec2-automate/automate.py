...
class EC2Instance:
    ...
    def __init__(self, inst_wrapper, key_name, sg_wrapper, ssm_client):
        """
    ...
    def create_security_group(self,sg_name,port):
        """
        ...
        response = self.sg_wrapper.authorize_ingress(current_ip_address,port)
        if response['Return']:
    ...
    def create_instance(self,image_id,instance_type,key_pair_name):
        """
    ...
    def start_client(self,client_name,port):
        logging.basicConfig(level=logging.INFO, format='%(levelname)s: %(message)s')

        print('-'*88)
        print("Your EC2 is starting.")
        print('-'*88)

        #self.create_and_list_key_pairs()
        self.create_security_group(client_name,port)
        self.create_instance('ami-08f5f8ed43b9c257c',"t2.micro",'client')
...
if __name__ == '__main__':
    try:
        scenario = EC2Instance(InstanceWrapper.from_resource(),'client',SecurityGroupWrapper.from_resource(),boto3.client('ssm'))
        scenario.start_client('client',22)
        scenario.remove_client()
    except Exception:
